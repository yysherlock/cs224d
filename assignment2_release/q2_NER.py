...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      window = tf.nn.embedding_lookup(embedding, self.input_placeholder) # same way as numpy indices, (data_size, window_size, embed_size)
  ...
  def add_loss_op(self, logits):
    ...
    """
    ### YOUR CODE HERE
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits, self.labels_placeholder))
